...
#define ABS2(x) (dble(x)**2 + IMAG(x)**2)
...
#if defined(DEBUG) && (! defined(ORACLE) )
...
#define SAFE_ALLOCATE_CLASS(cls,x) \
allocate(cls::x,stat=alc); \
call alloc_check(alc,SIZEOF(x),#x,__FILE__,__LINE__,.true.)
...
#define SAFE_ALLOCATE_CLASS(cls,x) allocate(cls::x)
