!---------------------------------------------------------------------
subroutine my_set_irr_new(xq, u, npert, nirr, eigen) 
!---------------------------------------------------------------------
!
...
!
    ...
    DO imode = 1,3*nat
      ...
      IF(rap > 12) call errore('set_irr', 'problem with the representation', 1)
      !
...
!
    DO imode=1,3*nat-1
      DO jmode = imode+1, 3*nat
        IF( (num_rap_mode(imode) /= num_rap_mode(jmode)) .AND.  &
          ...
          WRITE(stdout,'("Eigenvectors exchange needed",2i5)') imode, jmode
          eig(1)=eigen(jmode)
          phi(:,1)=u(:,jmode)
  ...
  npert (1) = 1
  do imode = 2, 3 * nat
    if( abs(eigen(imode) - eigen(imode-1)) / (abs(eigen(imode)) + abs(eigen(imode-1))) < 1.d-4) then
  ...
  IF( search_sym .AND. nspin_mag /= 4) THEN
    ...
    name_rap_mode=' '
    DO imode = 1, 3*nat
      IF( num_rap_mode(imode) > 0 ) name_rap_mode(imode)=name_rap(num_rap_mode(imode))
    ENDDO
  ENDIF
