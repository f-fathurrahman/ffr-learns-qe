!-----------------------------------------------------------------------
SUBROUTINE my_solve_linter(irr, imode0, npe, drhoscf)
!-----------------------------------------------------------------------
  USE kinds,                ONLY : DP
  ...
  IF( rec_code_read > 20 ) then
    write(*,*) 'Early return in my_solve_linter'
    RETURN
  ENDIF
...
!
  ...
  nsolv = 1 ! ffr: This is the usual case
  ...
  if (doublegrid) then
    ...
    dvscfins => dvscfin ! ffr: huh? pointer?
  endif
  ...
  do kter = 1, niter_ph
    ...
    DO ik = 1, nksq
      ...
      ! ffr: this is only needed for noncolin (?)
      !
      DO isolv = 1, nsolv
        ! write(*,*) 'isolv = ', isolv
        IF( isolv == 2 ) THEN
          ! for noncollinear magn
          ikmk = ikmks(ik)
          ikmkmq = ikmkmqs(ik)
          rsign = -1.0_DP
        ELSE
          ! default case (collinear magn)
          ikmk = ikk
          ikmkmq = ikq
          rsign = 1.0_DP
        ENDIF
        ...
        do ipert = 1, npe
          mode = imode0 + ipert
          nrec = (ipert - 1) * nksq + ik + (isolv-1) * npe * nksq ! ffr: what's this?
          write(*,'(1x,A,4I4)') 'kter, ik, ipter, nrec = ', kter, ik, ipert, nrec
          !
    ...
    IF( okpaw ) THEN
      ...
      DO ipert=1,npe
        dbecsum(:,:,:,ipert) = 2.0_DP * dbecsum(:,:,:,ipert) + becsumort(:,:,:,imode0+ipert)
      ENDDO
    ELSE
...
155 iter0 = 0
  ...
  write(*,*)
  write(*,*) '</div> EXIT my_solve_linter'
  write(*,*)

  RETURN  

END SUBROUTINE my_solve_linter
