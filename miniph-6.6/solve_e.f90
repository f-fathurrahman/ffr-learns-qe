...
subroutine solve_e
  ...
  do kter = 1, niter_ph
    ...
    do ik = 1, nksq
      ...
      do ipol = 1, 3
        ...
        IF( iter > 1 ) THEN
          ...
          !
          CALL adddvscf(ipol,ik) ! this is defined in LR_Modules
          !
    ...
    !
    DO ipol = 1,3
      IF(fildrho /= ' ') CALL davcio_drho(dvscfout(1,1,ipol), lrdrho, iudrho,ipol, +1)
      IF(lnoloc) THEN
...
!
    ...
    rec_code=-20
    IF (okpaw) THEN
     CALL write_rec('solve_e...', 0, dr2, iter, convt, 3, dvscfin, dvscfout, dbecsum)
    ELSE
