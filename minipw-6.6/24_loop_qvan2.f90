!-----------------------------------------------------------------------
SUBROUTINE test_loop_qvan2( ngy, ih, jh, np, qmod, qg, ylmk0 )
!-----------------------------------------------------------------------
  ...
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: ngy !! number of G vectors to compute
  INTEGER, INTENT(IN) :: ih !! first index of Q
  INTEGER, INTENT(IN) :: jh !! second index of Q
  INTEGER, INTENT(IN) :: np !! index of pseudopotentials
  REAL(DP), INTENT(IN) :: ylmk0(ngy,lmaxq*lmaxq) !! spherical harmonics
  REAL(DP), INTENT(IN) :: qmod(ngy) !! moduli of the q+g vectors 
  REAL(DP), INTENT(OUT) :: qg(2,ngy) !! the Fourier transform of interest
  !
  ! ... local variables
  !
  REAL(DP) :: sig
  ...
  jvl = nhtolm(jh,np)
  !
  IF( nb > nbetam .OR. mb > nbetam ) THEN 
    CALL errore( ' qvan2 ', ' wrong dimensions (1)', MAX(nb,mb) )
  ENDIF
  !
  IF( ivl > nlx .OR. jvl > nlx ) THEN
  ...
  DO lm = 1, lpx(ivl,jvl)
    ...
    !  IF( ABS( qmod(ig) - qm1 ) > 1.0D-6 ) THEN ! XXX need to check this?
    !    qm = qmod(ig) * dqi
    !    px = qm - INT(qm)
    !    ux = 1.0_DP - px
    !    vx = 2.0_DP - px
