!-----------------------------------------------
subroutine my_radial_gradient(f,gf,r,mesh,iflag)
!-----------------------------------------------
  !
  ...
  !write(*,*) 'Using coarse mesh for radial gradient'
  ...
  enddo
  return
end subroutine my_radial_gradient


!--------------------------------------------
subroutine my_fit_pol(xdata,ydata,n,degree,b)
!--------------------------------------------
  !
  ...
  if (info.ne.0) call errore('pol_fit','problems with the linear system', abs(info))
  return
end subroutine my_fit_pol
