...
!-----------------------------------------------------------------------
  ...
  symm: DO i = 1, 3 !emine: if it is not resolved in 3 steps it is sth else?
    !
    write(*,*) 'find_sym: Trying to find symmetry with accep = ', accep
    !
    CALL sgam_at( nat, tau, ityp, sym, no_z_inv )
    write(*,*) 'after sgam_at: count(sym) = ', count(sym)
    !
    ! Here we check for magnetic symmetries
    IF ( magnetic_sym ) then
      CALL sgam_at_mag( nat, m_loc, sym )
      write(*,*) 'after sgam_at_mag: count(sym) = ', count(sym)
    endif
    ...
    WRITE(*,*) 'before copy_sym: nsym = ', nsym
    write(*,*) 'before copy_sym: nrot = ', nrot
    write(*,*) 'before copy_sym: count(sym) = ', count(sym)
    nsym = copy_sym( nrot, sym )

    WRITE(*,*) 'after copy_sym: nsym = ', nsym

    !
    IF ( .NOT. is_group( nsym ) ) THEN
      !WRITE(*,*) 'Pass here 428'
      IF (i == 1) then
        write(*,*) 'find_sym: Not a group! Trying with lower acceptance parameter...'
      endif
      accep = accep * 0.5d0
      IF( i == 3 ) THEN
        write(*,*) 'find_sym: Still not a group! symmetry disabled'
        nsym = 1
      ENDIF
      CYCLE symm
    ELSE
      !WRITE(*,*) 'Pass here 437'
      IF(i > 1) then
        write(*,*) 'find_sym: Symmetry operations form a group, will exit the loop'
      endif
      EXIT symm
    ENDIF
  ENDDO symm

  WRITE(*,*) 'after loop symm: nsym = ', nsym
  ...
  WRITE(*,*) 'at the end of find_sym: nsym = ', nsym

  WRITE(*,*)
  WRITE(*,*) '</div> EXIT find_sym'
  WRITE(*,*)
...
!-----------------------------------------------------------------------
  ...
  WRITE(*,*) 'count(sym) = ', count(sym)
  write(*,*) 'nsym_ns = ', nsym_ns
  WRITE(*,*) 'fft_fact = ', fft_fact

  WRITE(*,*)
  WRITE(*,*) '</div> EXIT sgam_at'
  WRITE(*,*)
  !
...
!-----------------------------------------------------------------------
  ...
  write(*,*)
  write(*,*) '<div> ENTER sgam_at_mag'
  write(*,*)

  write(*,*) 'at the beginning: count(sym) = ', count(sym)
  write(*,*) 'no_t_rev = ', no_t_rev
  write(*,*) 'm_loc = ', m_loc
  write(*,*) 'bg(1,:) = ', bg(1,:)
  write(*,*) 'bg(2,:) = ', bg(2,:)
  write(*,*) 'bg(3,:) = ', bg(3,:)
  ...
  DO na = 1, nat
    mxau(:,na) = bg(1,:) * m_loc(1,na) + &
                  bg(2,:) * m_loc(2,na) + &
                  bg(3,:) * m_loc(3,na)
    write(*,*) 'ia = ', na, ' mxau = ', mxau(:,na)
  ENDDO
  !
  DO irot = 1, nrot
    !
    write(*,*)
    write(*,*) 'begin irot = ', irot
    t_rev(irot) = 0
    !
    IF ( sym(irot) ) THEN
      !
      ...
      DO na = 1, nat
          mrau(:,na) = s(1,:,irot) * mxau(1,na) + &
                      s(2,:,irot) * mxau(2,na) + &
                      s(3,:,irot) * mxau(3,na)
          write(*,*) 'ia = ', na, ' mrau = ', mrau(:,na)
      ENDDO
      !
      IF (sname(irot)(1:3) == 'inv') then
        write(*,*) 'inversion symmetry: set mrau to -mrau'
        mrau = -mrau
      endif
      !
      ...
      DO na = 1, nat
        ...
        !
        ss1 = sum(ABS(mrau(1:3,na) - mxau(1:3,nb)))
        ss2 = sum(ABS(mrau(1:3,na) + mxau(1:3,nb)))
        ...
        write(*,*) 'ss=', ss1, ' ss2 = ', ss2, ' t1 = ', t1, ' t2 = ', t2
        !
      ENDDO
      !
...
!-----------------------------------------------------------------------
  ...
  !
  CALL set_sym_bl()
  CALL find_sym( nat, tau, ityp, .NOT.time_reversal, m_loc, .false. )
  ! no_z_inv is set to false
  !
  RETURN
  !
...
!---------------------------------------------------------------------
  ...
  ENDIF
  !
  WRITE(*,*) 'At the end of set_sym_bl: nrot = ', nrot
  !
  write(*,*)
  write(*,*) '</div> EXIT my_set_sym_bl'
  write(*,*)
