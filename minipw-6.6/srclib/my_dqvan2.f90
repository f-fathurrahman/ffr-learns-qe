...
!-----------------------------------------------------------------------
  ...
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: ngy !! the number of G vectors to compute
  INTEGER, INTENT(IN) :: ih !! first index of Q(ih,jh)
  INTEGER, INTENT(IN) :: jh !! second index of Q(ih,jh)
  INTEGER, INTENT(IN) :: np !! pseudopotential index
  INTEGER, INTENT(IN) :: ipol !! the polarization of the derivative
  REAL(DP), INTENT(IN) ::  g(3,ngy) !! G vectors
  REAL(DP), INTENT(IN) ::  tpiba !! 2pi/a factor, multiplies G vectors
  REAL(DP), INTENT(IN) ::  qmod(ngy) !! moduli of q+G vectors
  REAL(DP), INTENT(IN) ::  ylmk0(ngy,lmaxq*lmaxq) !! spherical harmonics
  REAL(DP), INTENT(IN) ::  dylmk0(ngy,lmaxq*lmaxq) !! derivetives of spherical harmonics
  COMPLEX(DP), INTENT(OUT) :: dqg(ngy) !! the fourier transform of interest
  !
  ! ... local variables
  !
  COMPLEX(DP) :: sig
