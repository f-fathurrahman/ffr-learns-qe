!----------------------------------------------------------------------------
SUBROUTINE my_gradcorr( rho, rhog, rho_core, rhog_core, etxc, vtxc, v )
!----------------------------------------------------------------------------
  ...
  DO is = 1, nspin0
     ...
     ! ffr: core density is added (what about fac?)
     ! ffr: core density is always calculated for paired-electrons configuration
     ! ffr: divide by 2 in case of spin pol calculation
     !
  ...
  ELSE
    ...
    DO k = 1, dfftp%nnr
      ...
      ENDDO
      !
      vtxcgc = vtxcgc + e2 * ( v1x(k,1) + v1c(k,1) ) * ( rhoaux(k,1) - rho_core(k) * fac )
      vtxcgc = vtxcgc + e2 * ( v1x(k,2) + v1c(k,2) ) * ( rhoaux(k,2) - rho_core(k) * fac )
      etxcgc = etxcgc + e2 * ( sx(k) + sc(k) )
      !
    ENDDO
    !
