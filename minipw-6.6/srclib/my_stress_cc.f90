!-----------------------------------------------------------------------
subroutine my_stress_cc( sigmaxcc )
!-----------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
...
15 continue
  ...
  do nt = 1, ntyp
    if( upf(nt)%nlcc ) then
      call drhoc(ngl, gl, omega, tpiba2, msh(nt), rgrid(nt)%r, rgrid(nt)%rab, upf(nt)%rho_atc, rhocg)
      ! diagonal term
  ...
  write(*,*)
  write(*,*) 'Core charge stress, not symmetrized (Ry/bohr**3):'
  write(*,*)
  do l = 1,3
    write(*,'(1x,3F18.10)') sigmaxcc(l,1), sigmaxcc(l,2), sigmaxcc(l,3)
  enddo

  write(*,*)
  write(*,*) '*** EXIT my_stress_cc'
  write(*,*)


  return
end subroutine my_stress_cc
