!----------------------------------------------------------------------
subroutine my_stress_loc( sigmaloc )
!----------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  ...
  do nt = 1, ntyp
    ...
    ELSE
      ...
      !
      call my_dvloc_of_g( rgrid(nt)%mesh, msh(nt), rgrid(nt)%rab, rgrid(nt)%r, &
  ...
  write(*,*)
  write(*,*) 'Local potential stress, not symmetrized (Ry/bohr**3):'
  write(*,*)
  do l = 1,3
    write(*,'(1x,3F18.10)') sigmaloc(l,1), sigmaloc(l,2), sigmaloc(l,3)
  enddo

  write(*,*)
  write(*,*) '*** EXIT my_stress_loc'
  write(*,*)

  return
end subroutine my_stress_loc
