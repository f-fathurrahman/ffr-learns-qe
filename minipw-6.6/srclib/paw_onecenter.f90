...
MODULE paw_onecenter
  ...
  PRIVATE
  !
  REAL(DP), ALLOCATABLE   :: msmall_lm(:,:,:) !! magnetiz. due to small components expanded on Y_lm
...
!
! Called by hinit1, electrons_scf, potinit
!
!---------------------------------------------------------------------------------
          ...
          DO nb = 1, nh(i%t)
            DO mb = nb, nh(i%t)
              ...
              IF (i_what == AE) THEN
                ...
                IF (with_small_so) CALL PAW_rho_lm( i, becfake, upf(i%t)%paw%pfunc_rel, msmall_lm )
              ELSE
...
!--------------------------------------------------------------------
  ...
  DO ix = ix_s, ix_e
    ...
    IF( nspin_mag==4 ) THEN
      ...
      !
      DO k = 1, i%m
        IF( PRESENT(energy) ) e_rad(k) = e2*(ex(k)+ec(k))*(rho_rad(k,1) + rho_core(k)*g(i%t)%r2(k))
        !
  ...
  ELSEIF ( nspin_mag == 2 .OR. nspin_mag == 4 ) THEN
    ...
    DO ix = ix_s, ix_e
      ...
      DO k = 1, i%m
        ...
        h_rad(k,:,ix,1) =( (v2x(k,1) + v2c(k,1))*grad(k,:,1) + v2cud(k)*grad(k,:,2) )*g(i%t)%r2(k)
        h_rad(k,:,ix,2) =( (v2x(k,2) + v2c(k,2))*grad(k,:,2) + v2cud(k)*grad(k,:,1) )*g(i%t)%r2(k)
        !
      ENDDO
      !
      ! integrate energy (if required)
...
SUBROUTINE PAW_gradient( i, ix, rho_lm, rho_rad, rho_core, grho_rad2, grho_rad )
  ...
  DO is = 1, nspin_gga
    ...
    grho_rad2(1:i%m,is) = grho_rad2(1:i%m,is) &
                            + (aux(1:i%m)**2 + aux2(1:i%m)**2) * g(i%t)%rm2(1:i%m)**3
...
!------------------------------------------------------------------------------------
  ...
  !
  !
  !write(*,*) '********* Calling PAW_rho_lm' ! many calls
