...
SUBROUTINE readpp ( input_dft, printout, ecutwfc_pp, ecutrho_pp )
  ...
  !USE emend_upf_module, ONLY: make_emended_upf_copy  ! ffr
  ...
  DO nt = 1, ntyp
     ...
     ELSE
        ...
        ELSE IF ( upf_get_pp_format( psfile(nt) ) == 5 ) THEN
           ...
           IF( printout_ ) &
            ...
            write(*,*) 'Pseudofile is not supported'
            stop
     ...
     !write(*,*) 'Pass here 239 in read_pseudo'
     !write(*,*) 'upf(nt)%q_with_l = ', upf(nt)%q_with_l
     !if(.not. upf(nt)%q_with_l) then
     !  write(*,*) 'qfunc  = ', upf(nt)%qfunc(10,1)
     !else
     !  write(*,*) 'qfuncl = ', upf(nt)%qfuncl(10,1,1)
     !endif
