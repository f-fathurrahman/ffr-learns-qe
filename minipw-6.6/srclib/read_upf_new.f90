...
!---------------------------------------------+
  ...
  IF(upf%nlcc) then
    CALL xmlr_readtag( capitalize_if_v2('pp_nlcc'), upf%rho_atc(:) )
  else
    upf%rho_atc(:) = 0.0_dp
  endif
  ...
  IF( .NOT. upf%tcoulombp) then
    ...
    CALL xmlr_readtag( capitalize_if_v2('pp_local'), upf%vloc(:), ierr )
    !
  ...
  CALL xmlr_readtag( capitalize_if_v2('pp_rhoatom'), upf%rho_at(1:upf%mesh) )
  !
