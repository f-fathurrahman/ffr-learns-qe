...
!------------------------------------------------------------------------------
  ...
  IF(ionode) THEN
    WRITE(stdout,*)
    WRITE(stdout,*) 'Calling dbg_electrons'
    WRITE(stdout,*) '*********************'
  ENDIF
  ...
  IF(ionode) WRITE(stdout,*) 'exst = ', exst


  niter = 20  ! overwritten for debugging purpose
  ...
  DO idum=1,niter
    ...
    END IF
    IF(ionode) WRITE(stdout,*) 'ethr = ', ethr
    ...
    deband_hwf = delta_e()
    IF(ionode) WRITE(stdout,*) 'deband_hwf = ', deband_hwf
    ...
    scf_step: DO
      ...
      IF(ionode) WRITE(stdout,*) 'ethr, tr2_min = ', ethr, tr2_min
